...
{
    gpio_num_t gpio_num = (gpio_num_t)(uint8_t)arg;

    button_event_data_t event_data = {
        .gpio_num = gpio_num,
        .event = BUTTON_EVENT_PRESSED,
        .timestamp = esp_timer_get_time() // em MS
    };

    static uint64_t last_isr_time[2] = {0, 0};
    int button_index = (gpio_num == BUTTON_NAV_GPIO) ? 0 : 1;

    // debounce
    uint64_t current_time = esp_timer_get_time();

    if ((current_time - last_isr_time[button_index]) < BUTTON_DEBOUNCE_TIME_US)
    {
        return;
    }
...
int button_read_debounced(gpio_num_t gpio_num)
{
    ...
    if (current_state != last_state[gpio_num] &&
        (current_time - last_read_time[gpio_num]) > BUTTON_DEBOUNCE_TIME_US)
    {
