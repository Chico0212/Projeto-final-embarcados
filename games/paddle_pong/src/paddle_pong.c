...
void update_paddle() {
    ...
    if (mpu6050_read_acceleration(&ax, &ay, &az) == ESP_OK) {
        ...
        int new_x = game.paddle.x + (int)(ax * 15); // Sensibilidade ajustável
        
        // Limitar dentro da tela
        if (new_x < 0) new_x = 0;
        if (new_x > SSD1306_WIDTH - PADDLE_WIDTH) new_x = SSD1306_WIDTH - PADDLE_WIDTH;
...
void update_ball() {
    ...
    if (game.ball.x <= 0 || game.ball.x >= SSD1306_WIDTH - game.ball.size) {
        ...
        if (game.ball.x <= 0) game.ball.x = 0;
        if (game.ball.x >= SSD1306_WIDTH - game.ball.size) game.ball.x = SSD1306_WIDTH - game.ball.size;
    }
    ...
    if (check_paddle_collision() && game.ball.vy > 0) {
        ...
        if (game.ball.vx > 3.0f) game.ball.vx = 3.0f;
        if (game.ball.vx < -3.0f) game.ball.vx = -3.0f;
    }
...
void draw_game() {
    ...
    if (game.game_over) {
        ...
        ssd1306_draw_string(10, 35, score_str);
        ssd1306_draw_string(5, 50, "Incline para");
        ssd1306_draw_string(15, 58, "reiniciar");
    } else {
        // Desenhar raquete
        ...
        char hud_str[32];
        snprintf(hud_str, sizeof(hud_str), "score: %d", game.score);
        ssd1306_draw_string(0, 1, hud_str);
        
        snprintf(hud_str, sizeof(hud_str), "lives: %d", game.lives);
        ssd1306_draw_string(80, 0, hud_str);
        
        // Desenhar linha separando o HUD
...
void game_task(void *pvParameters) {
    ...
    while (1) {
        if (game.game_over) {
            ...
            if (check_restart()) {
                game_init();
                vTaskDelay(pdMS_TO_TICKS(1000)); // Evitar reinício acidental
            }
        } else {
            // Atualizar jogo
            update_paddle();
